#!/usr/bin/env python3
"""
Bulk Transition Dev Workflow Script
        ...
        if response.status_code == 200:
            ...
            for comment in comments:
                if qe_comment_text.lower() in comment.get('body', '').lower():
                    print(f"✅ Found QE comment on {ticket_id}")
                    return True
            
            print(f"❌ QE comment not found on {ticket_id}")
            return False
        else:
            print(f"❌ Failed to get comments for {ticket_id}: {response.status_code} {response.text}")
            return False
            
    except Exception as e:
        print(f"❌ Error checking comments for {ticket_id}: {str(e)}")
        return False
        ...
        if response.status_code == 200:
            ...
            for attachment in attachments:
                ...
                if qe_evidence_pattern.search(filename):
                    print(f"✅ Found QE-Evidence attachment on {ticket_id}: {filename}")
                    return True
            
            print(f"❌ QE-Evidence attachments not found on {ticket_id}")
            return False
        else:
...
def validate_ticket_for_transition(url, token, ticket_id, target_status):
    ...
    if target_status in ["Acceptance", "Done"]:
        print(f"🔍 Validating requirements for {ticket_id} -> {target_status}")
        ...
        print(f"✅ All requirements met for {ticket_id}")
...
def main():
    """Main function to handle bulk dev workflow transitions with validation."""
    
    parser = argparse.ArgumentParser(
        description="Transition dev tickets through workflow: Inprogress->Review->Acceptance->Done with validation",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Transition dev tickets with validation
  python bulk_transition_dev_workflow.py --excel tickets.xlsx --url https://jira.company.com --token your_token
  
  # Dry run to see what would be transitioned
  python bulk_transition_dev_workflow.py --excel tickets.xlsx --dry-run --url https://jira.company.com --token your_token
  
  # Use configuration defaults
  python bulk_transition_dev_workflow.py --excel tickets.xlsx --use-config
        """
    )
    ...
    print("🔧 Bulk Dev Workflow Transition Tool (with Validation)")
    print("=" * 70)
