#!/usr/bin/env python3
"""
Bulk Transition Dev Workflow Script (Sync Version)
Transition tickets through the dev workflow: Inprogress->Review->Acceptance->Done
Uses ticket URLs from "Security Ticket" column of Excel.
Validates that "QE Testing in Dev Completed" comment exists and QE-Evidence POC is uploaded.
...
def check_qe_comment_exists(url, token, ticket_id):
    """
    Check if "QE Testing in Dev Completed" comment exists on the ticket.
    ...
    """
    try:
        api_url = get_issue_url(url, ticket_id) + "?expand=comments"
        headers = get_jira_headers(token)
        ...
        if response.status_code == 200:
            ...
            qe_comment_text = "QE Testing in Dev Completed"
            
            for comment in comments:
                if qe_comment_text.lower() in comment.get('body', '').lower():
                    info_log(f"Found QE comment on {ticket_id}")
                    return True
            
            error_log(f"QE comment not found on {ticket_id}")
            return False
        else:
            error_log(f"Failed to get comments for {ticket_id}: {response.status_code} {response.text}")
            return False
            
    except Exception as e:
        error_log(f"Error checking comments for {ticket_id}: {str(e)}")
        return False


def check_qe_evidence_attachments(url, token, ticket_id):
    """
    Check if QE-Evidence attachments exist on the ticket.
    ...
    Returns:
        bool: True if QE-Evidence attachments exist, False otherwise
    """
    try:
        api_url = get_issue_url(url, ticket_id) + "?expand=attachment"
        headers = get_jira_headers(token)
        ...
        if response.status_code == 200:
            ...
            qe_evidence_pattern = re.compile(r'QE-Evidence-', re.IGNORECASE)
            
            for attachment in attachments:
                filename = attachment.get('filename', '')
                if qe_evidence_pattern.search(filename):
                    info_log(f"Found QE-Evidence attachment on {ticket_id}: {filename}")
                    return True
            
            error_log(f"QE-Evidence attachments not found on {ticket_id}")
            return False
        else:
...
def determine_next_status(current_status):
    """
    Determine the next status in the dev workflow.
    ...
    """
    workflow = ["Inprogress", "Review", "Acceptance", "Done"]
...
def validate_ticket_for_transition(url, token, ticket_id, target_status):
    """
    Validate that a ticket meets requirements for transition.
    ...
    """
    # For transitions to Acceptance and Done, check requirements
    if target_status in ["Acceptance", "Done"]:
        info_log(f"Validating requirements for {ticket_id} -> {target_status}")
        
        # Check for QE comment
        has_qe_comment = check_qe_comment_exists(url, token, ticket_id)
        
        # Check for QE-Evidence attachments
        has_qe_evidence = check_qe_evidence_attachments(url, token, ticket_id)
        
        if not has_qe_comment:
            return False, "Missing QE Testing in Dev Completed comment"
        
        if not has_qe_evidence:
            return False, "Missing QE-Evidence attachments"
        
        success_log(f"All requirements met for {ticket_id}")
        return True, "All requirements met"
    
    # For other transitions, no special requirements
    return True, "No special requirements"


def process_dev_transitions(df, url, token, security_ticket_col="Security Ticket", 
                           dry_run=False):
    """
    Process Excel rows and transition dev tickets through workflow with validation.
    ...
    for idx, row in filtered_df.iterrows():
        ...
        if dry_run:
            ...
            row_dict['validation_status'] = 'DRY_RUN'
            ...
            if current_status is None:
                ...
                row_dict['validation_status'] = 'Failed to get status'
            ...
            if next_status is None:
                ...
                row_dict['validation_status'] = 'N/A'
            ...
            # Validate requirements for transition
            is_valid, validation_message = validate_ticket_for_transition(url, token, ticket_id, next_status)
            
            if not is_valid:
                error_log(f"Validation failed: {validation_message}")
                # Add row with validation failure info
                row_dict = row.to_dict()
                row_dict['current_status'] = current_status
                row_dict['next_status'] = next_status
                row_dict['validation_status'] = validation_message
                row_dict['transition_status'] = 'Validation failed'
                row_dict['transition_date'] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                updated_rows.append(row_dict)
                continue
...
def update_database_with_transitions(df, db_path, table_name="tickets"):
    ...
    try:
        ...
        new_columns = ['current_status', 'next_status', 'validation_status', 'transition_status', 'transition_date']
...
def main():
    """Main function to handle bulk dev workflow transitions with validation and sync."""
    
    parser = argparse.ArgumentParser(
        description="Transition dev tickets through workflow: Inprogress->Review->Acceptance->Done with validation (with Excel/DB sync)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Transition dev tickets with validation and sync
  python bulk_transition_dev_workflow_sync.py --excel tickets.xlsx --url https://jira.company.com --token your_token --db tickets.db
  
  # Dry run to see what would be transitioned
  python bulk_transition_dev_workflow_sync.py --excel tickets.xlsx --dry-run --url https://jira.company.com --token your_token --db tickets.db
  
  # Use configuration defaults
  python bulk_transition_dev_workflow_sync.py --excel tickets.xlsx --use-config --db tickets.db
        """
    )
    ...
    print("🔧 Bulk Dev Workflow Transition Tool (with Validation and Sync)")
    print("=" * 80)
    print(f"Excel file: {args.excel}")
    print(f"Security Ticket column: {args.security_ticket_col}")
    print(f"Workflow: Inprogress -> Review -> Acceptance -> Done")
    print(f"Validation: QE comment and QE-Evidence attachments required for Acceptance/Done")
    if args.db:
    ...
    try:
        ...
        # Process dev transitions
        success_count, total_processed, updated_df = process_dev_transitions(
        ...
        if args.dry_run:
            ...
            print(f"✅ Dev workflow transitions complete! {success_count}/{total_processed} transitions successful")
