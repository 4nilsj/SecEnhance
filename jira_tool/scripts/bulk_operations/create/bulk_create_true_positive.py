...
def main():
    """Main function to handle bulk creation of true positive tickets."""
    
    parser = argparse.ArgumentParser(
        description="Create Jira tickets only for true positive findings with empty Security Ticket column",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Create tickets for true positive findings
  python bulk_create_true_positive.py --excel findings.xlsx --project SEC --url https://jira.company.com --token your_token
  
  # Create tickets with custom column names
  python bulk_create_true_positive.py --excel findings.xlsx --project SEC --status-col "finding_status" --security-ticket-col "Jira_Ticket" --url https://jira.company.com --token your_token
  
  # Dry run to see what would be created
  python bulk_create_true_positive.py --excel findings.xlsx --project SEC --dry-run --url https://jira.company.com --token your_token
  
  # Create tickets with specific issue type and priority
  python bulk_create_true_positive.py --excel findings.xlsx --project SEC --issue-type "Bug" --priority "High" --url https://jira.company.com --token your_token
        """
    )
    ...
    parser.add_argument("--issue-type", default="Task", help="Default issue type (default: Task)")
    ...
    ticket_config = get_ticket_creation_config()
    ...
    if args.use_config or not args.issue_type:
        args.issue_type = args.issue_type or ticket_config["issue_type"]["name"]
    if args.use_config or not args.priority:
        args.priority = args.priority or ticket_config["priority"]
    ...
    print("🔧 Bulk Create True Positive Tickets Tool")
    print("=" * 50)
