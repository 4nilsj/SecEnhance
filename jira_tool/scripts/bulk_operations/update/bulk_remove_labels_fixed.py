#!/usr/bin/env python3
"""
Bulk Remove Labels from Jira Tickets
...
def remove_labels_from_ticket(url, token, ticket_key, labels_to_remove):
    ...
    try:
        ...
        if len(updated_labels) == len(current_labels):
            print(f"ℹ️  No labels to remove for ticket {ticket_key}")
            return True
        ...
        if response.status_code == 204:
            ...
            print(f"✅ Successfully removed labels from ticket {ticket_key}: {removed_labels}")
            print(f"   Remaining labels: {updated_labels}")
            return True
        else:
            print(f"❌ Failed to remove labels from ticket {ticket_key}: {response.status_code} {response.text}")
            return False
            
    except Exception as e:
        print(f"❌ Error removing labels from ticket {ticket_key}: {str(e)}")
        return False


def process_remove_labels(excel_file, sheet_name, dev_ticket_col, labels_to_remove, url, token):
    """
    Process label removal for tickets from Excel file.
    ...
    try:
        ...
        for index, row in df_filtered.iterrows():
            ...
            # Remove labels
            if remove_labels_from_ticket(url, token, ticket_key, labels_to_remove):
                successful += 1
            else:
                failed += 1
...
def main():
    """Main function for bulk label removal."""
    parser = argparse.ArgumentParser(description="Remove labels from Jira tickets using Excel data")
    ...
    print("🗑️  Bulk Label Removal Tool")
    print("=" * 40)
