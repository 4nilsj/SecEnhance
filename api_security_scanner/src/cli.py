...
def scan(ctx, input_file, curl_command, auth_type, auth_name, auth_value, 
         ...
         performance_stats, no_zap, no_plugins, spider_depth, spider_children, max_scan_time):
    ...
    try:
        ...
        echo(f"Parsed {len(requests_data)} requests")
        ...
        if auth_type and auth_name and auth_value:
            try:
                ...
                echo(f"Authentication configured: {auth_type} - {auth_name}")
        ...
        if not no_plugins:
            ...
            echo(f"Loaded {len(plugin_manager.loaded_plugins)} custom plugins")
                ...
                with zap_manager:
                    ...
                    if spider_success:
                        ...
                        if scan_success:
                            ...
                            echo(f"Found {len(zap_alerts)} ZAP alerts")
                        else:
                            echo("Active scan failed", err=True)
                    else:
                        echo("Spidering failed", err=True)
                ...
                for result in plugin_results:
                    if result.success:
                        for finding in result.findings:
                            ...
                            custom_alerts.append(finding)
                    else:
                        logger.warning(f"Plugin {result.plugin_name} failed: {result.error}")
                
                echo(f"Custom plugins found {len(custom_alerts)} issues")
        ...
        if export or export_json:
            ...
            if export:
                ...
                if success:
                    echo(f"HTML report saved to: {export}")
                else:
            ...
            if export_json:
                ...
                if success:
                    echo(f"JSON report saved to: {export_json}")
                else:
