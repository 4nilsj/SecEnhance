...
async function analyzeTarget(url, options) {
  ...
  try {
    ...
    inlineScripts.forEach((s, idx) => {
      ...
      analyzeJSContent(content, srcLabel, stores);
    });
    ...
    const tasks = scriptUrls.map(src => async () => {
      try {
        ...
        analyzeJSContent(jsText, src, stores);     // advanced detections
      } catch (err) {
...
// Read targets: single URL, file list, or stdin
...
program
  .name("web-recon")
  .description("Extract endpoints, parameters, target-origin JS files, technologies and perform advanced JS security analysis from target URL(s)")
  .argument("[url]", "Target URL to analyze (optional if using -l or --stdin)")
  ...
  .action(async (urlArg, options) => {
    ...
    if (!targets.length) {
      console.error(chalk.red("No targets provided. Use a URL, -l <file>, or --stdin."));
      process.exit(1);
    }
